...
function pdf( x, options ) {
	...
	opts.alpha = typeof opts.alpha !== 'undefined' ? opts.alpha : 1;
	opts.beta = typeof opts.beta !== 'undefined' ? opts.beta : 1;

	if ( isNumber( x ) ) {
		return pdf1( x, opts.alpha, opts.beta );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return pdf5( out, x, opts.alpha, opts.beta );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.alpha, opts.beta );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.alpha, opts.beta, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.alpha, opts.beta, opts.accessor );
		}
		return pdf2( out, x, opts.alpha, opts.beta );
	}
	return NaN;
} // end FUNCTION pdf()
