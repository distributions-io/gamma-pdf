...
* @param {Number} [options.beta=1] - rate parameter
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'alpha' ) ) {
		...
		if ( !isPositive( opts.alpha ) ) {
			return new TypeError( 'pdf()::invalid option. `alpha` parameter must be a positive number. Option: `' + opts.alpha + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'beta' ) ) {
		opts.beta = options.beta;
		if ( !isPositive( opts.beta ) ) {
			return new TypeError( 'pdf()::invalid option. `beta` parameter must be a positive number. Option: `' + opts.beta + '`.' );
		}
	}
